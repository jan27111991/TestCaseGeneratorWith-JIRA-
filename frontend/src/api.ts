...
export async function connectJira(config: JiraConfig): Promise<void> {
...
export async function fetchJiraStories(): Promise<JiraStory[]> {
...
export async function fetchJiraStoryDetails(storyKey: string): Promise<JiraStory> {
  const response = await fetch(`${API_BASE_URL}/jira/stories/${storyKey}`)
