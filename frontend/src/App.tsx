...
import { generateTests, connectJira, fetchJiraStories, fetchJiraStoryDetails } from './api'
import { GenerateRequest, GenerateResponse, TestCase, JiraConfig, JiraStory, JiraConnection } from './types'
import { JiraConnectionModal } from './components/JiraConnectionModal'
...
function App() {
  ...
  const handleStorySelect = async (storyKey: string) => {
    try {
      const story = await fetchJiraStoryDetails(storyKey)
      setSelectedStory(story)
      
      // Small helper to extract text from ADF (Atlassian Document Format)
      const extractTextFromADF = (value: any): string => {
        if (!value) return ''
        if (typeof value === 'string') return value
        if (Array.isArray(value)) return value.map((v: any) => extractTextFromADF(v)).join('\n')
        const walk = (node: any): string => {
          if (!node) return ''
          if (node.type === 'text') return node.text || ''
          if (Array.isArray(node.content)) return node.content.map((c: any) => walk(c)).join('')
          return ''
        }
      ...
      const extractADFSection = (root: any, headingTitle: string): string => {
        if (!root) return ''
      ...
      // Populate form with story details (convert any ADF objects to plain text)
      const descriptionText = extractTextFromADF(story.fields.description)
      const acFieldText = extractTextFromADF(story.fields.customfield_10001)
      const acFromDesc = extractADFSection(story.fields.description, 'Acceptance Criteria')

      setFormData({
        storyTitle: story.fields.summary,
        description: descriptionText || '',
        acceptanceCriteria: acFieldText || acFromDesc || '',
        additionalInfo: ''
      })
    } catch (err) {
    ...
    <div>
      ...
      <div className="container">
        <div className="header">
          ...
          <div className="jira-controls" style={{ marginTop: '20px', display: 'flex', gap: '10px', justifyContent: 'center' }}>
            {!jiraConnection.isConnected ? (
              <button
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                >
                  ...
                  {jiraConnection.stories.map(story => (
        ...
        <div className="content-layout" style={{ display: 'flex', gap: '20px' }}>
          <form onSubmit={handleSubmit} className="form-container" style={{ flex: '1' }}>
